...
export default class Network {
  /**
   *
   * @param {Object|{ countOfInputNeurons: number, countOfHiddenNeurons:  number, countOfOutputNeurons: number }} options
   */
  constructor(options = { countOfInputNeurons: 3, countOfHiddenNeurons: 2, countOfOutputNeurons: 1 }) {
      /**
       * - 0 input
       *          - 0 hidden
       * - 0 input              - 0 output
      ...
      this.settings = {
          ...
          countOfInputNeurons: objectPath.get(options, ['countOfInputNeurons'], 3),
          // 2. кол-во скрытых нейронов
          countOfHiddenNeurons: objectPath.get(options, ['countOfHiddenNeurons'], 2),
          // 2. кол-во нейронов на выхлде
          countOfOutputNeurons: objectPath.get(options, ['countOfOutputNeurons'], 1)
      }
       ...
       * @type {number[][]}
       */
      this.weightsInput = this.randomInputWeights(-1, 1)

      // this.weightsInput = [[.79, .44, .43, .49], [.85, .43, .29, .40], [.22, .63, .49, .46]]
       ...
       * @type {number[][]}
       */
      this.weightsHidden = this.randomHiddenWeights(-1, 1)

      // this.weightsHidden = [[.5, .52, .48]]
  }
  ...
  predict(inputs) {
      ...
      // console.log('input', inputs, input1, output1) //+
